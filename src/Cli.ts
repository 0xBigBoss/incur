...
async function serveImpl(
  ...
  if (llms) {
    ...
    let scopedLeaf: CommandDefinition<any, any, any> | undefined
    for (const token of filtered) {
      ...
      if (isGroup(entry)) {
        ...
        if (!isFetchGateway(entry)) scopedLeaf = entry
        scopedPath = [...prefix, token]
        break
      }
    }
    ...
    if (scopedLeaf && scopedPath) {
      const described = describeCommand(scopedPath, scopedLeaf)
      writeln(Formatter.format(described, formatFlag))
      return
    }
