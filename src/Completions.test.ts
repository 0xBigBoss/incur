...
describe('register', () => {
  test('bash: generates complete -F script with nospace support', () => {
    ...
    expect(script).toContain('_COMPLETE_INDEX="$_COMPLETE_INDEX"')
    expect(script).toContain('if [[ $? != 0 ]]; then')
    expect(script).toContain('if [[ "$_c" == *$\'\\001\' ]]; then')
    expect(script).toContain('COMPREPLY+=("$_c")')
    expect(script).toContain('if [[ $_nospace == true ]]; then')
  ...
  test('zsh: generates compdef script', () => {
    ...
    expect(script).toContain('if [[ -n $completions ]]; then')
