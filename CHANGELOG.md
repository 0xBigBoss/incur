...
- aff345e: fix(SyncSkills): `sync.include` now fully overrides a command-generated
...
- 73a64d9: fix(CommandOptions): revert runtime dry-run auto-derivation
...
- 67721eb: fix(Parser): accept `--no-<x>` as a literal `noX` boolean field when `--<x>` is not registered
